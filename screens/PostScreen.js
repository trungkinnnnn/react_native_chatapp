...
const CreatePostScreen = ({ route }) => {
 // const userEmail = "hoa@gmail.com";
  ...
  const handlePost = async(userEmail)=>{
    ...
    const timestamp_post = new Date().getTime();
    const times_str = `${timestamp_post}`
  
    const postsCollection = collection(database, 'posts');
    const postUser = doc(postsCollection,userEmail);
    let img_post;
    if(postImage)
    {
       img_post = await uploadImage(postImage,times_str);
    }else{
      img_post = ""
    }
    ...
    const newPostDoc = await setDoc(doc(collection(postUser, 'user_posts'), times_str), {
      content: postText,
      image: img_post,
        ...
        <View style={styles.conten}>
          ...
          <View style={styles.header}>
            ...
            <TouchableOpacity 
                ...
                >
                <Text style={styles.text_save}>Đăng bài</Text>
              </TouchableOpacity>
          </View>
